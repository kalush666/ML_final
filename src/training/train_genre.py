...
class GenreTrainingPipeline:
    ...
    def prepare_datasets(self, use_subset: bool = False, subset_size: int = 2000):
        ...
        if use_subset:
            ...
            train_gen.df = train_gen.df.head(subset_size)
    ...
    def run(self, epochs: int = 50, batch_size: int = 32, learning_rate: float = 0.001, use_subset: bool = False, subset_size: int = 2000):
        print("="*70)
        print(" "*20 + "GENRE CLASSIFIER TRAINING")
        print("="*70)

        if use_subset:
            self.prepare_datasets(use_subset=True, subset_size=subset_size)
        else:
